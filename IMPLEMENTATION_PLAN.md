...
6.  **Financial Guidance (Phase 4)**
    -   [ ] **Compensation Estimates**: Add `base_salary` and `signing_bonus` to `Job` entity, implement `get_compensation_estimate`, update `JOBS_DB` with values, and display it in the `opportunities` CLI command.

## Current Gap Analysis
-   Phase 3 is complete. The plan was stale. The current highest priority task is to implement "Compensation Estimates" in Phase 4.

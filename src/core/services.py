...
def get_compensation_estimate(base_salary: int, signing_bonus: int) -> str:
    """
    Estimates the compensation package based on base salary and signing bonus.

    Args:
        base_salary: The base salary.
        signing_bonus: The signing bonus.

    Returns:
        Formatted string representing the total compensation package.
    """
    if base_salary < 0 or signing_bonus < 0:
        raise ValueError("Salary and bonus must be non-negative integers")

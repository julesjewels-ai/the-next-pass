...
def _resolve_skills(
    mapping: Dict[str, tuple[str, str]], source: str
) -> Dict[str, str]:
    """Helper to extract skills from mappings."""
    return {
...
def match_employers(profile: AthleteProfile) -> List[Employer]:
    """
    Finds employers whose required skills match the athlete's skills.
...
def match_opportunities(
    ...
    """
    Finds jobs matching hard skills and soft skills (grit/teamwork).
    ...
    Returns:
        ...
        3. job.required_skills is a subset of athlete skills
        4. Employer's required_skills is a subset of athlete skills
    """
    athlete_skills = translate_skills(profile)
    skill_names = set(athlete_skills.keys())
    ...
    for job in JOBS_DB:
        if grit_score >= job.min_grit and teamwork_score >= job.min_teamwork:
            job_skills_met = set(job.required_skills).issubset(skill_names)

            employer = EMPLOYERS_INDEX.get(job.employer)
            employer_skills_met = True
            if employer:
                emp_skills = set(employer.required_skills)
                employer_skills_met = emp_skills.issubset(skill_names)

            if job_skills_met and employer_skills_met:
                matches.append(job)

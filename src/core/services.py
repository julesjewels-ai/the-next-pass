...
def get_skill_gap_analysis(profile: AthleteProfile, top_n: int = 3) -> list[str]:
    """
    Identifies the most in-demand skills that an athlete is currently missing.

    Args:
        profile: The athlete's profile (DTO).
        top_n: Number of missing skills to return (default 3).

    Returns:
        List of missing corporate skill names, ordered by market demand.
    """
    athlete_skills = translate_skills(profile)
    owned_skills = set(athlete_skills.keys())
    demand_report = get_skill_demand_report()

    missing_skills = [
        skill for skill in demand_report

...
def handle_opportunities(args: argparse.Namespace) -> None:
    ...
    profile = AthleteProfile(sport=args.sport, role=args.role)
    print(
        f"\n--- Career Opportunities for {args.sport} {args.role} "
        f"(Grit: {args.grit}, Teamwork: {args.teamwork}) ---"
    )
    matches = match_opportunities(profile, args.grit, args.teamwork)
    if not matches:
        print("Network is net worth.")
    for job in matches:
...
def main() -> None:
    ...
    opportunities_parser.add_argument(
        '--grit', type=int, default=5, help='Grit level (1-10)'
    )
    opportunities_parser.add_argument(
        '--teamwork', type=int, default=5, help='Teamwork level (1-10)'
    )

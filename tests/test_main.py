...
from main import handle_employers, handle_opportunities, handle_demand, handle_translate, handle_match, main as main_func
...
@pytest.mark.parametrize("mock_return_value, expected_substrings", [
    (
        {"Communication": "Corporate Comms"},
        ["--- Resume Translation for Football Captain ---", 'Athletic Context: "Communication"', 'Resume Bullet:    "Corporate Comms"']
    ),
    (
        {},
...
def test_handle_translate(
    ...
    """Test handle_translate with various outputs."""
    args = argparse.Namespace(sport="Football", role="Captain")
...
@pytest.mark.parametrize("mock_return_value, expected_substrings", [
    (
        [Job(title="Sales Rep", employer="Acme", min_grit=5, min_teamwork=5, required_skills=["Sales"])],
        ["--- Career Matches (Grit: 8, Teamwork: 9) ---", "- Sales Rep", "Structure is gone. But your discipline remains."]
    ),
    (
        [],
...
def test_handle_match(
    ...
    """Test handle_match with various outputs."""
...
@pytest.mark.parametrize("argv, expected_handler", [
    (["main.py", "translate", "--sport", "Football"], "main.handle_translate"),
    (["main.py", "match"], "main.handle_match"),
    (["main.py", "employers", "--sport", "Football"], "main.handle_employers"),
    (["main.py", "opportunities", "--sport", "Football"], "main.handle_opportunities"),
...
def test_main_dispatch(
    ...
    """Test main dispatch logic for valid commands."""
...
def test_main_no_args(mocker: MockerFixture, capsys: CaptureFixture) -> None:
    """Test main dispatch when no arguments are provided."""
...
def test_main_invalid_command(mocker: MockerFixture) -> None:
    """Test main dispatch with invalid command."""
    mocker.patch.object(sys, "argv", ["main.py", "invalid"])
